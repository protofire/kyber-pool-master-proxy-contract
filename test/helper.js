...
module.exports.sendEtherWithPromise = function (sender, recv, amount) {
  return new Promise(function (fulfill, reject) {
    web3.eth.sendTransaction({to: recv, from: sender, value: amount}, function (
      error,
      result
    ) {
      if (error) {
...
module.exports.exp = function (num1, num2) {
  const num1Math = Math.bignumber(new BN(num1 * 10 ** 9).toString(10)).div(
    10 ** 9
  );
  const num2Math = Math.bignumber(new BN(num2 * 10 ** 9).toString(10)).div(
    10 ** 9
  );
...
module.exports.ln = function (num) {
  const numMath = Math.bignumber(new BN(num * 10 ** 9).toString(10)).div(
    10 ** 9
  );
...
function checkAbsDiff(num1, num2, maxDiffInPercentage) {
  ...
  return diff
    .mul(new BN(100).div(new BN(num1)))
    .lte(new BN(maxDiffInPercentage * 100));
}
...
module.exports.assertAbsDiff = function (
  val1,
  val2,
  expectedDiffInPct,
  errorStr
...
function assertEqual(val1, val2, errorStr) {
  assert(
    new BN(val1).should.be.a.bignumber.that.equals(new BN(val2)),
    errorStr
  );
}
...
module.exports.assertGreater = function (val1, val2, errorStr) {
  assert(
    new BN(val1).should.be.a.bignumber.that.is.greaterThan(new BN(val2)),
    errorStr
  );
};
...
module.exports.assertLesser = function (val1, val2, errorStr) {
  assert(
    new BN(val1).should.be.a.bignumber.that.is.lessThan(new BN(val2)),
    errorStr
  );
};
...
module.exports.calcSrcQty = function (dstQty, srcDecimals, dstDecimals, rate) {
  ...
  if (srcDecimals.gte(dstDecimals)) {
    numerator = precisionUnits
      .mul(dstQty)
      .mul(new BN(10).pow(new BN(srcDecimals.sub(dstDecimals))));
    denominator = new BN(rate);
  } else {
    numerator = precisionUnits.mul(dstQty);
    denominator = new BN(rate).mul(
      new BN(10).pow(new BN(dstDecimals.sub(srcDecimals)))
    );
  }
...
module.exports.calcDstQty = function (srcQty, srcDecimals, dstDecimals, rate) {
  ...
  if (dstDecimals.gte(srcDecimals)) {
    ...
    result = srcQty
      .mul(rate)
      .div(
        precisionUnits.mul(new BN(10).pow(new BN(srcDecimals.sub(dstDecimals))))
      );
  }
  return result;
};

module.exports.assertSameEtherBalance = async function (
  accountAddress,
  expectedBalance
) {
...
module.exports.assertSameTokenBalance = async function (
  accountAddress,
  token,
  expectedBalance
) {
...
module.exports.calcRateFromQty = function (
  srcQty,
  dstQty,
  srcDecimals,
  dstDecimals
  ...
  return new Promise((resolve, reject) => {
    web3.currentProvider.send.bind(web3.currentProvider)(
      ...
      (err, res) => {
        ...
        console.log(
          `next block timestamp will be: ${currentChainTime + duration}`
        );
        resolve(res);
      }
    );
